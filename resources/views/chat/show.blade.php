    ...
    <div class="max-w-4xl mx-auto flex flex-col h-[calc(100vh-120px)]">
            ...
            @foreach ($messages as $message)
                ...
                <div class="flex {{ $isCurrentUser ? 'justify-end' : 'justify-start' }}">
                    <div class="flex max-w-[75%] flex-col gap-1 {{ $isCurrentUser ? 'items-end' : 'items-start' }}">
                        <div class="rounded-2xl px-4 py-2.5 text-sm leading-relaxed {{ $isCurrentUser ? 'bg-gradient-to-br from-emerald-500 to-teal-500 text-white' : 'bg-white text-slate-700 shadow-sm' }}">
                            <p>{{ $message->body }}</p>
                        </div>
                        <div class="flex items-center gap-2 text-[10px] text-slate-400">
                            <span>{{ optional($message->sent_at)->format('H:i') }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="border-t border-slate-200 bg-white px-4 py-3">
            <div class="flex items-center gap-3">
                ...
                <form action="{{ route('chat.store') }}" method="POST" class="flex items-center gap-3 flex-1">
                    @csrf
